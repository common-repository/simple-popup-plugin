...
Author URI: https://grimmdude.com
*/
...
add_action('widgets_init', 'simple_popup_Widget_init', 1);

function simple_popup_menu() {
    add_options_page('Simple Popup Plugin Options', 'Simple Popup Plugin', 'administrator', __FILE__, 'simple_popup_options');
}
...
function popup_plugin_shortcode( $atts, $content = null ) {
	...
	return sprintf(
		'<a href="%s" onclick="return popitup(this.href, %s, %s);" class="simple_popup_link %s">%s</a>',
		esc_attr($url),
		esc_attr($width),
		esc_attr($height),
		esc_attr($class),
...
function simple_popup_link( $templateurl, $link_text ) {
	echo sprintf('<a href="%s" onclick="return popitup(this.href);" class="simple_popup_link">%s</a>', $templateurl, $link_text);
}
