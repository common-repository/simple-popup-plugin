...
class simple_popup_Widget extends WP_Widget {
    ...
    function __construct() {
        ...
        parent::__construct( 'simple_popup', __('Simple Popup Plugin'), $widget_ops );
    }
        ...
        if ( $instance['urls'] ) {
            ...
            if (is_array($urls)) {
                foreach ($urls as $pop) {
                    if ( !empty( $pop['url'] ) ) {
                        $out .= sprintf(
                            '<li><a href="%s" onClick="return popitup(this.href, %s, %s);" target="_blank">%s</a></li>',
                            esc_attr($pop['url']),
                            esc_attr($pop['width']),
                            esc_attr($pop['height']),
                            esc_html($pop['clicktext'])
                        );
                        $out .= "\n";
                    }
                }
